...
public class RegisterOpenMRSPayload {
    ...
    public static void patient(AMRSPatientServices amrsPatientServices, AMRSIdentifiersService amrsIdentifiersService,AMRSPersonAtrributesService amrsPersonAtrributesService,String kenyaemrLocationUuid,String url,String auth) throws JSONException, ParseException, IOException {
        //OpenMRS Payload

        List<AMRSPatients> amrsPatientsList = amrsPatientServices.findByResponseCodeIsNull();
        if(!amrsPatientsList.isEmpty()) {
            for (int xx = 0; xx < amrsPatientsList.size(); xx++) {

                AMRSPatients amrsPatients = amrsPatientsList.get(xx);
                //identifier
                ...
                //check if exists
                List<AMRSIdentifiers> OpenMRSidentifiers = amrsIdentifiersService.findByPatientidAndIdentifierType(amrsPatients.getPersonId(),"OPENMRS ID");
                if (OpenMRSidentifiers.isEmpty()) {
                    ...
                    ai.setIdentifierType("OpenMRS ID");
                    amrsIdentifiersService.save(ai);
                }

                for (int x = 0; x < identifiers.size(); x++) {

                    System.out.println("AMRS ID before Error " + amrsPatients.getPersonId() + " " + identifiers.get(x).getIdentifier() + " Type Failing " + identifiers.get(x).getIdentifierType());
                ...
                JSONArray namearray = new JSONArray();
                JSONObject jsonNames = new JSONObject();
                jsonNames.put("givenName", escapeSpecialCharacters(amrsPatients.getGiven_name()));
                jsonNames.put("familyName", escapeSpecialCharacters(amrsPatients.getFamily_name()));
                jsonNames.put("middleName", escapeSpecialCharacters(amrsPatients.getMiddle_name()));
                namearray.put(jsonNames);
                // End of dentifier
                JSONArray jsonAddressesArray = new JSONArray();
                JSONObject jsonAddresses = new JSONObject();
                ...
                System.out.println("Response ndo hii " + responseBody + " More message " + response.message() + " reponse code " + response.code());
               // System.out.println("Patient object "+patientObject.toString());
                JSONObject jsonObject = new JSONObject(responseBody);
